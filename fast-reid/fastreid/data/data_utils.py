...
class DataLoaderX(DataLoader):
    def __init__(self, local_rank, **kwargs):
        ...
        # On Windows or when CUDA is unavailable, skip stream creation
        ...
        try:
            ...
            # If stream creation fails, continue without it
            pass

    def __iter__(self):
        self.iter = super().__iter__()
        self.batch = None  # Initialize batch attribute
        
        # On Windows or without CUDA, skip background generator
        # to avoid multiprocessing issues
        import sys
        if torch.cuda.is_available() and sys.platform != 'win32':
            self.iter = BackgroundGenerator(self.iter, self.local_rank)
        
        # Always preload the first batch (needed for __next__ to work)
        self.preload()
        return self
    ...
    def preload(self):
        ...
        if self.stream is not None:
            ...
            # Without CUDA stream, just move tensors normally
            for k in self.batch:
                if isinstance(self.batch[k], torch.Tensor):
    ...
    def __next__(self):
        # Only wait on stream if it exists
        if self.stream is not None:
        ...
        # Always load next batch (preload works with or without BackgroundGenerator)
        self.preload()
        return batch
